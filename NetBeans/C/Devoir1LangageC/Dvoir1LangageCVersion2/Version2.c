...
int main(int argc, char** argv)
{
    ...
    while(val1!=valAlea1 || val2!=valAlea2 || val3!=valAlea3 || val4!=valAlea4){
        ...
        //Dire quelles valeurs sont incorrectes et lesquelles sont correctes
        if(val1!=valAlea1){
            //printf("Votre première valeur est incorrecte.\n");
            printf("Donnez la première valeur : ");
            scanf("%d",&val1);            
        }
        else{
            //printf("Votre première valeur est correcte.\n");
            nbDeValeursCorrectesBienPlacees=nbDeValeursCorrectesBienPlacees+1;
        }
        if(val2!=valAlea2){
            //printf("Votre seconde valeur est incorrecte.\n");
            printf("Donnez la seconde valeur : ");
            scanf("%d",&val2);
        }
        else{
            //printf("Votre seconde valeur est correcte.\n");
            nbDeValeursCorrectesBienPlacees=nbDeValeursCorrectesBienPlacees+1;
        }
        if(val3!=valAlea3){
            //printf("Votre troisième valeur est incorrecte.\n");
            printf("Donnez la troisième valeur : ");
            scanf("%d",&val3);
        }
        else{
            //printf("Votre troisième valeur est correcte.\n");
            nbDeValeursCorrectesBienPlacees=nbDeValeursCorrectesBienPlacees+1;
        }
        if(val4!=valAlea4){
            //printf("Votre quatrième valeur est incorrecte.\n");
            printf("Donnez la quatrième valeur : ");
            scanf("%d",&val4);
        }
        else{
            //printf("Votre quatrième valeur est correcte.\n");
            nbDeValeursCorrectesBienPlacees=nbDeValeursCorrectesBienPlacees+1;
            
        }
        if(nbDeValeursCorrectesBienPlacees>0){
            printf("Vous avez %d valeurs correctes qui sont bien placées.\n",nbDeValeursCorrectesBienPlacees);
        }
        else{
            printf("Vous avez aucune valeur correcte bien placée.");
        }
        //Vérification que les valeurs du joueur soient comprises entre 0 et 9
        while(0>val1 || val1>9){
    ...
    printf("Vous avez gagnez. Il vous a fallu %d tentative(s).",nbDeTentative);
